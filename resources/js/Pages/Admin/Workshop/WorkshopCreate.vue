      ...
      <div class="page-card">
        <div class="form" action="">
          <div class="form">
            ...
            <!-- <div class="error" v-if="errors.thumbnail">サムネイル 必須項目です。</div>
            <div class="error" v-if="errors.thumbnail_invalid">
              ファイル形式をjpg,jpeg,png,gif,svgのみです。
            </div>
            <div class="error" v-if="errors.thumbnail_size">
...
資料ダウンロードの説明文</textarea
                    ...
                    <img
                      @click="removeFromAssignmentArea(assignment.id)"
  ...
  methods: {
    submitAndPublish(status) {
      ...
      if (this.validate() && this.validateDatesAndTimes()) {
        this.form.post(route("admin.workshop.store"), {
          onSuccess: () => {
            console.log("success");
          },
          onFinish: () => {
