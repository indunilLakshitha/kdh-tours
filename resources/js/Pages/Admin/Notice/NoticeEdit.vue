        ...
        <div class="form" action="">
          <div class="form">
            <div class="form-group sml">
              <label>タイトル<span>(50文字以内)</span></label>
              <div class="w-100">
                <input
                  type="text"
                  v-model="form.title"
...
export default {
  props: { notice_categories: Object, notice: Object },
  ...
  created() {
    this.form.title = this.notice.title;
    this.form.id = this.notice.id;
    this.form.link = this.notice.link ? this.notice.link : "";
    this.notice.categories.forEach((e) => {
      this.category_component.push({
        id: this.category_component_id++,
        cat_id: e.category.id,
        name: e.category.category,
  ...
  methods: {
    ...
    submit(status) {
      ...
      if (!this.errors.categories && !this.errors.title && !this.errors.link) {
        this.form.post(route("admin.notice.update"), {
          onSuccess: () => {},
          onFinish: () => {},
        });
      }else{
